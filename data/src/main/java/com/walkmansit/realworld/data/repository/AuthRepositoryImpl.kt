...
import com.walkmansit.realworld.data.model.response.LoginFailedResponse
import com.walkmansit.realworld.data.model.response.RegistrationFailedResponse
import com.walkmansit.realworld.data.model.response.toDomain
import com.walkmansit.realworld.data.model.response.toLoginFailed
import com.walkmansit.realworld.data.model.response.toRegistrationFailed
...
class AuthRepositoryImpl @Inject constructor(
    ...
    override suspend fun login(userCredentials: UserLoginCredentials): Either<Either<CommonError, LoginFailed>, User> {
        return try {
            ...
            val mapper =
                object : ModelsMapper<LoginFailedResponse, LoginFailed> {
                    override fun map(data: LoginFailedResponse): LoginFailed = data.toLoginFailed()
                }
    ...
    override suspend fun register(
        ...
        return try {
            ...
            val body = e.response()?.errorBody()?.string() ?: ""
            val mapper =
                object : ModelsMapper<RegistrationFailedResponse, RegistrationFailed> {
                    override fun map(data: RegistrationFailedResponse): RegistrationFailed = data.toRegistrationFailed()
                }
