...
sealed interface TagsIntent : MVIIntent {
        data class UpdateSearchQuery(val query: String) : TagsIntent
        data class SubmitNewTag(val tag: String) : TagsIntent
        data class TagsLoaded(val result: Either<Boolean, List<String>>) : TagsIntent
        data class DeleteTag(val tag: String) : TagsIntent
        data class AddTag(val tag: String) : TagsIntent
}

sealed interface TagState : MVIState {
        data object Loading : TagState
        data class Error(val message: String) : TagState
        data class Content(val content: TagFields = TagFields()) :  TagState
}
...
class TagsViewModel @Inject constructor(
...
) : ViewModel(), Container<TagState,TagsIntent, TagAction> {
        override val store = store(initial = TagState.Loading, scope = viewModelScope){
                configure {
                        name = "TagStore"
                        debuggable = true
        ...
        private val _selectedTags = mutableSetOf<String>()
        private val _allTags = mutableSetOf<String>()
        ...
        private suspend fun TagsCtx.submitNewTag(tag: String){
                if (tag.isNotEmpty()
                        && _selectedTags.none { it.lowercase() == tag }
                        && _allTags.none { it.lowercase() == tag }
                ){
                        updateStateOrThrow<TagState.Content, _> {
                                _selectedTags.add(tag)
                                with(content){
        ...
        private fun filterTags(query:String, tags: List<String>) : List<String> {
                return tags.filter { tag -> tag.contains(query) }
        }
        ...
        private suspend fun TagsCtx.onTagsLoaded(tagsResult: Either<Boolean, List<String>>){
                updateStateOrThrow<TagState.Loading, _> {
                        when(tagsResult){
                                ...
                                is Either.Success -> {
                                        _allTags.addAll(tagsResult.value )
                                        TagState.Content(
                                                content = TagFields(
...
//                                                copy(content = copy(
//                                                        allTags = tagsResult.value,
//                                                        searchResult = filterTags(content.searchQuery.text, content.allTags)
//                                                ))
//                                        }
